...
function switchCharacter(characterNumber) {
  console.log(`Switching to character ${characterNumber}`);
  currentCharacter = characterNumber;

  document.getElementById(
    "characterSwitcher"
  ).style.transform = `translateX(-${(currentCharacter - 1) * 60}px)`;

  document.querySelectorAll(".character").forEach((character) => {
    character.style.width = "200px";
    character.style.height = "250px";
    character.classList.remove("selected");
    character.classList.remove("unselected");
  });

  document
    .querySelector(`.character:nth-child(${currentCharacter})`)
    .classList.add("selected");

  document
    .querySelectorAll(".character:not(.selected)")
    .forEach((character) => {
      character.classList.add("unselected");
    });
}

document.addEventListener("keydown", function (event) {
  if (event.key === "ArrowLeft") {
    currentCharacter = ((currentCharacter - 2) % 6) + 1;
    if (currentCharacter < 1) {
      currentCharacter = 6;
    }
    switchCharacter(currentCharacter);
  } else if (event.key === "ArrowRight") {
    currentCharacter = (currentCharacter % 6) + 1;
    switchCharacter(currentCharacter);
  }
});
